  ...
  return (
    <section className="border-b border-border px-6 md:px-14 py-16 md:py-24">
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_360px] gap-12 lg:gap-20 items-end">
        <div ref={mainReveal.ref} className={`reveal ${mainReveal.isVisible ? 'visible' : ''}`}>
        ...
        <div ref={sidebarReveal.ref} className={`border-t lg:border-t-0 lg:border-l border-border pt-8 lg:pt-0 lg:pl-12 flex flex-col reveal ${sidebarReveal.isVisible ? 'visible' : ''}`}>
          {stats.map((stat, index) => (
            <div key={stat.label} className={`py-5 border-b border-border ${index === 0 ? 'border-t' : ''}`}>
